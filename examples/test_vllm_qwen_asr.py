...
# 测试qwen_asr支持的长度
def test_length_url(asr: Qwen3ASRModel) -> None:
    
    url_list = ["audio_data/1min.wav", "audio_data/2min.wav", "audio_data/3min.wav", "audio_data/4min.wav", "audio_data/5min.wav", "audio_data/10min.wav"]
    for index, cur_url_zh in enumerate(url_list):
        start_time = time.time()
        results = asr.transcribe(
            audio=cur_url_zh,
            language=None,
        ...
        cost_time = time.time() - start_time
        print(f"{index+1}: {cur_url_zh} 识别时间: {cost_time:.2f}s")
        assert isinstance(results, list) and len(results) == 1
        _print_result("single-url (no forced language, no timestamps)", results)
...
def test_batch_mixed(asr: Qwen3ASRModel) -> None:
    ...
    for batch_num in batch_list:
        start_time = time.time()
        results = asr.transcribe(
            audio=[zh_b64] * batch_num,
            context=[""] * batch_num,
            language=["Chinese"] * batch_num,
            return_time_stamps=False,
        )
        assert len(results) == batch_num
        cost_time = time.time() - start_time
        print(f"batch_num: {batch_num}, cost_time: {cost_time}")
